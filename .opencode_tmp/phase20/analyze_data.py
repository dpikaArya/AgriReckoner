...
yield_vars = [
    c
    for c in u21["Variable"].unique()
    if any(k in str(c).lower() for k in ["yield", "biomass", "height", "protein", "spad"])
]
print("\n== Yield-ish variables present ==")
for v in sorted(yield_vars):
    sub = u21[u21["Variable"] == v]
    print(
        f"  {v:<30} rows={len(sub):<6} units={sorted(sub['NormalizedUnit'].dropna().unique())[:6]}"
    )
...
ext = pd.read_parquet(
    C.PROJECT_ROOT / "outputs" / "phase18" / "staging" / "external_observations.parquet"
)
...
print(
    "  with lat:",
    ext["Location_Lat"].notna().sum(),
    " with lon:",
    ext["Location_Lon"].notna().sum(),
)
...
print(
    ll.groupby([ll["Location_Lat"].round(2), ll["Location_Lon"].round(2)])
    .size()
