...
def compute_independence(obs: pd.DataFrame) -> pd.DataFrame:
    ...
    out["repeated_cultivars"] = out.groupby("Crop_Normalized")[
        "ObservationID_ML"].transform("count")

    # independent grouping identifiers
    out["IndependentGroup_Observation"] = out["canonical_observation_id"]
    out["IndependentGroup_Treatment"] = out["canonical_experiment_id"].astype(str) + "::" + \
        out["TreatmentID"].astype(str)
...
def run(force: bool = False):
    ...
    keep = ["ObservationID_ML", "canonical_observation_id", "canonical_study_id",
            ...
            "ExperimentID", "TreatmentID", "Crop", "Crop_Normalized", "year",
            "observations_per_study", "observations_per_experiment",
            "observations_per_location", "observations_per_paper",
            "observations_per_treatment", "repeated_measurements",
            "repeated_locations", "repeated_cultivars",
            "IndependentGroup_Observation", "IndependentGroup_Treatment",
            "IndependentGroup_Experiment", "IndependentGroup_Study",
    ...
    C.log_msg(f"STEP14 independence: {result['independent_studies']} studies, "
