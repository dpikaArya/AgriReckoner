...
PREDICTOR_UAMS_COLUMNS = [
    "Average_Temperature", "Temperature_Max", "Temperature_Min", "Rainfall",
    "Humidity", "Wind_Speed", "Solar_Radiation", "Organic_Carbon", "Soil_pH",
    "Soil_Clay_Pct", "Soil_Sand_Pct", "Soil_Silt_Pct", "Soil_Bulk_Density",
    "Nitrogen", "CEC", "Soil_Moisture",
]
...
def link_predictors(obs: pd.DataFrame, cfg: dict,
                    spatial: pd.DataFrame | None = None) -> pd.DataFrame:
...
def build_predictor_matrix(link: pd.DataFrame) -> pd.DataFrame:
    ...
    wide = link.pivot_table(
        index=["ObservationID_ML", "canonical_observation_id",
               "canonical_location_id", "Crop", "Year"],
        columns="predictor", values="value", aggfunc="first").reset_index()
...
def run(force: bool = False):
    ...
    result = {
        ...
        "observations_with_predictor_link": int(link["ObservationID_ML"].nunique()) if len(link) else 0,
        ...
        "predictors_linked": sorted(link["predictor"].dropna().unique().tolist()) if len(link) else [],
