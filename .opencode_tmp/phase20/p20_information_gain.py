...
SOURCE_RELIABILITY = {
    "CHIRPS": 0.9, "NASA_POWER": 0.9, "SoilGrids": 0.85, "FAOSTAT": 0.9,
    "MapSPAM": 0.7,
}

FEASIBILITY = {
    "Rainfall": 0.9, "Temperature": 0.9, "Temperature_Max": 0.9,
    "Temperature_Min": 0.9, "Soil_pH": 0.8, "Organic_Carbon": 0.8,
    "Nitrogen": 0.8, "Phosphorus": 0.3, "Potassium": 0.3, "CEC": 0.8,
    "Solar_Radiation": 0.85, "Humidity": 0.85, "Wind_Speed": 0.85,
    "Soil_Texture": 0.8, "Soil_Moisture": 0.3, "Plant_Population": 0.4,
    "Season": 0.5, "Location": 0.6, "Year": 0.9, "Crop": 0.9, "Cultivar": 0.5,
}

COST = {
    "Rainfall": 0.2, "Temperature": 0.2, "Temperature_Max": 0.2,
    "Temperature_Min": 0.2, "Soil_pH": 0.3, "Organic_Carbon": 0.3,
    "Nitrogen": 0.3, "Phosphorus": 0.5, "Potassium": 0.5, "CEC": 0.3,
    "Solar_Radiation": 0.2, "Humidity": 0.2, "Wind_Speed": 0.2,
    "Soil_Texture": 0.3, "Soil_Moisture": 0.5, "Plant_Population": 0.6,
    "Season": 0.6, "Location": 0.5, "Year": 0.1, "Crop": 0.1, "Cultivar": 0.5,
}
...
def compute_info_gain(obs: pd.DataFrame, cfg: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for var, grp in ig_df.groupby("MissingPredictor"):
        ...
        priority = "HIGH" if n_complete > 0 else ("MEDIUM" if n_missing > len(yield_obs) / 2 else "LOW")
        agg_rows.append({
            "MissingPredictor": var,
...
def run(force: bool = False):
    ...
    result = {
        ...
        "highest_gain_sources": sorted({
            s for v in (agg.head(10)["SourceAvailability"].tolist() if len(agg) else [])
            for s in v.split(";") if s != "none_approved"}),
    ...
    C.log_msg(f"STEP12/18 infogain: {result['observations_completable']} "
              f"observations completable")
    return result
