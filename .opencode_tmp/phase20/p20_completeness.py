...
def build_missingness(obs: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    out["SpatialCompleteness"] = (
        out["location_precision"]
        .map(
            {
                "exact_coordinates": 1.0,
                "experimental_station": 0.9,
                "study_location": 0.8,
                "district": 0.6,
                "country_only": 0.4,
                "unknown": 0.0,
    ...
    out["TemporalCompleteness"] = (
        out["temporal_precision"]
        .map({"growing_season": 1.0, "year_level": 0.6, "unknown": 0.0})
        .fillna(0.0)
    )
...
def run(force: bool = False):
    ...
    result = {
        ...
        "missing_critical_predictors": sorted(
            yield_obs["MissingCriticalPredictors"]
            .map(lambda s: s.split(";") if s else [])
            .explode()
            .dropna()
            .unique()
            .tolist()
    ...
    C.log_msg(
        f"STEP11 missingness: {n_yield} yield observations, "
