...
for c in meta_cols:
    if c in ext.columns:
        n = ext[c].notna().sum()
        print(
            f"  {c:<18} nonnull={n}  uniq={ext[c].nunique()}  sample={list(ext[c].dropna().unique()[:4])}"
        )
...
wsext = ext[
    ext["Variable"].isin(
        ["Average_Temperature", "Soil_pH", "Organic_Carbon", "Rainfall", "Nitrogen"]
    )
]
print(
    wsext[
        ["ObservationID", "Variable", "NormalizedValue", "OriginalUnit", "ConfidenceScore"]
    ].to_string()
)
