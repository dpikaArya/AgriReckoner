...
DERIVED_OF_YIELD = [
    "Harvest_Index", "Harvest_Index_Calc", "Yield_per_Plot", "Yield_per_Acre",
    "Yield_per_Plant", "Yield_per_Hectare_Calc", "Yield_per_Plot_Calc",
    "Economic_Yield", "Marketable_Yield", "Nitrogen_Use_Efficiency",
    "Water_Use_Efficiency", "Biomass_Yield",
]

POST_HARVEST = [
    "Protein", "Ash", "Fat", "Carbohydrates", "Fiber", "Oil", "Starch",
    "Moisture_Content", "100_Seed_Weight", "Grain_Quality", "Hardness",
]

RECOMMENDATION_DERIVED = ["N_Recommendation", "P_Recommendation",
                          "K_Recommendation", "Fertilizer_Recommendation",
                          "Dose_Recommendation", "Recommended_N",
                          "Recommended_P", "Recommended_K"]
...
def classify_variable(candidate: str, target: str) -> dict:
    ...
    t = target.strip()
    if c == t:
        return {"Candidate": c, "Target": t, "Class": "TARGET",
                "Rationale": "identical to target"}
    if c in POST_HARVEST:
        return {"Candidate": c, "Target": t, "Class": "LEAKAGE",
    ...
    if c.startswith(t) and len(c) > len(t):
        return {"Candidate": c, "Target": t, "Class": "LEAKAGE_RISK",
                "Rationale": "name suggests derivative of target"}
    if c in DERIVED_OF_YIELD:
        return {"Candidate": c, "Target": t, "Class": "CONDITIONALLY_SAFE",
                "Rationale": f"derivative of yield; safe only when target is a "
                            f"distinct non-yield trait ({t})"}
    return {"Candidate": c, "Target": t, "Class": "SAFE",
...
def run(force: bool = False):
    ...
    skip = {"ObservationID_ML", "PaperID", "ExperimentID", "TreatmentID",
            "ObservationID", "ObservationLevel", "canonical_observation_id",
            "canonical_study_id", "canonical_experiment_id", "canonical_location_id",
            "Provenance", "Source", "SourceSystem", "DOI", "n_measurement_rows",
            "Variety", "Crop", "Crop_Normalized", "Country", "State", "Site",
            "Institution", "Location", "Start_date", "End_date",
            "start_date", "end_date", "weather_period", "Season", "season",
            "year", "Canonical_Year", "Canonical_Crop", "Canonical_Location",
            "Canonical_Season", "Year", "Paper_Year", "QualityGrade", "Target",
            "ModelReadiness", "latitude", "longitude", "location_precision",
            "temporal_precision", "location_source", "location_confidence",
            "location_match_method", "administrative_level", "country", "state",
            "district", "sowing_month", "growing_days", "ProvenanceComplete",
            "SourceRow", "SourceColumn", "SourceTable", "Caption", "Dataset_ID",
            "Provenance_ID", "IdentityConfidence", "DuplicateCandidate"}
    ...
    pivot = summary.pivot_table(index="Candidate", columns="Class", values="n",
                                fill_value=0).reset_index()
    for col in ["SAFE", "CONDITIONALLY_SAFE", "LEAKAGE_RISK", "LEAKAGE", "TARGET"]:
        if col not in pivot.columns:
            pivot[col] = 0
    pivot["WorstClass"] = pivot.apply(
        lambda r: "TARGET" if r["TARGET"] else
        ("LEAKAGE" if r["LEAKAGE"] else
         ("LEAKAGE_RISK" if r["LEAKAGE_RISK"] else
          ("CONDITIONALLY_SAFE" if r["CONDITIONALLY_SAFE"] else "SAFE"))), axis=1)

    C.write_excel_multi({"variable_audit": var_audit,
                         "candidate_summary": pivot,
                         "observation_flags": obs_flag_df},
    ...
    result = {
        ...
        "structural_flags": {
            ...
            "recommendation_derived": int(obs_flag_df["LeakageFlags"].str.contains("recommendation_derived").sum()),
        },
