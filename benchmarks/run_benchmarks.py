...
def run_benchmarks():
    ...
    for crop, gs in GOLD_STANDARDS.items():
        ...
        var_precision = len(expected_vars & actual_vars) / len(expected_vars) if expected_vars else 1
        ...
        schema_precision = len(expected_uams & actual_uams) / len(expected_uams) if expected_uams else 1
        schema_recall = len(expected_uams & actual_uams) / len(expected_uams) if expected_uams else 1
        ...
        eng_f1 = 2 * eng_precision * eng_recall / (eng_precision + eng_recall) if (eng_precision + eng_recall) > 0 else 0
        ...
        if onto_total > 0:
            ...
            if onto_path.exists():
                ...
                if "Variable" in onto_df.columns and "AGROVOC_ID" in onto_df.columns:
                    mapped_vars = set(onto_df[onto_df["AGROVOC_ID"].notna() & (onto_df["AGROVOC_ID"] != "")]["Variable"])
    ...
    csv_data = []
    for r in results:
        for key in ["variable_precision", "variable_recall", "variable_f1",
                      "schema_f1", "ontology_coverage", "extraction_accuracy"]:
