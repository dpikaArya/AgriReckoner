...
from evaluation.utils import (
    OUTPUT_DIR, write_report, write_csv, get_master_df,
    safe_mean, MODEL_LIST, REPORTS_DIR,
)
import numpy as np
import pandas as pd
...
def _prepare_data():
    ...
    target_cols = [c for c in ["Target_Yield", "Target_Fertilizer", "Target_Nitrogen"] if c in df.columns]
    if not target_cols:
        return None, None, None
...
def benchmark_models():
    ...
    for model_name in ["Multiple Linear Regression", "Ridge Regression", "Lasso Regression",
                        "Elastic Net", "Random Forest", "Extra Trees",
    ...
    for model_name in ["XGBoost", "LightGBM", "CatBoost"]:
        try:
            if model_name == "XGBoost":
                ...
                model = CatBoostRegressor(n_estimators=50, max_depth=6, random_state=42, verbose=False)
    ...
    csv_path = write_csv("Model_Benchmark.csv", [
