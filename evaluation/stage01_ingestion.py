...
def evaluate_ingestion():
    ...
    for pdf_name in PAPER_FILES:
        ...
        refs_found = len(
            re.findall(r"(?:references|bibliography|works\s+cited)", text, re.IGNORECASE)
        )
    ...
    avg_completeness = (
        sum(r.get("text_extraction_completeness", 0) for r in results) / len(results)
        if results
        else 0
    )
    avg_table = (
        sum(r.get("table_extraction_accuracy", 0) for r in results) / len(results) if results else 0
    )
    avg_ocr = sum(r.get("ocr_quality", 0) for r in results) / len(results) if results else 0
    success_rate = (
        sum(1 for r in results if r.get("pdf_parsing_success")) / len(results) if results else 0
    )
