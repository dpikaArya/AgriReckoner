...
def n_adj(s):
    return cons_adj("Nitrogen_Adjustment", s)


def p_adj(s):
    return cons_adj("Potash_Adjustment", s)


def i_adj(s):
    return cons_adj("Irrigation_Adjustment", s)


def d_adj(s):
    return cons_adj("Dose_Adjustment", s)
...
for n_stat, _ in [("low", "Nitrogen")]:
    for env_fset, env_var in pH_sets + rain_sets + oc_sets + temp_sets:
        ...
        rule(
            rule_name,
            f"Nitrogen low + {env_var}={env_fset}",
            priority,
            [ant("Nitrogen", n_stat), ant(env_var, env_fset)],
            cons,
        )
priority -= 1
...
for env_fset, env_var in pH_sets + rain_sets + oc_sets + temp_sets:
    ...
    rule(
        rule_name,
        f"Phosphorus low + {env_var}={env_fset}",
        priority,
        [ant("Phosphorus", "low"), ant(env_var, env_fset)],
        cons,
    )
...
for env_fset, env_var in pH_sets + rain_sets + oc_sets + temp_sets:
    ...
    rule(
        rule_name,
        f"Potassium low + {env_var}={env_fset}",
        priority,
        [ant("Potassium", "low"), ant(env_var, env_fset)],
        cons,
    )
...
for n_stat, p_stat in np_combos:
    for env_fset, env_var in rain_sets[:2] + pH_sets[:2]:
        ...
        cons = [
            ...
            cons_nutrient(
                "Phosphorus", "increase" if p_stat == "low" else "maintain", p_amount, "%"
            ),
            cons_nutrient("Potassium", "maintain"),
        ...
        rule(
            rule_name,
            f"N={n_stat} P={p_stat} + {env_var}={env_fset}",
            priority,
            [ant("Nitrogen", n_stat), ant("Phosphorus", p_stat), ant(env_var, env_fset)],
            cons,
        )
...
for n_stat, k_stat in nk_combos:
    for env_fset, env_var in rain_sets[:2]:
        ...
        cons = [
            cons_nutrient("Nitrogen", "increase" if n_stat == "low" else "maintain", n_amount, "%"),
            cons_nutrient(
                "Potassium", "increase" if k_stat == "low" else "maintain", k_amount, "%"
            ),
            cons_nutrient("Phosphorus", "maintain"),
            n_adj("high" if n_stat == "low" else "medium"),
        ...
        rule(
            rule_name,
            f"N={n_stat} K={k_stat} + {env_var}={env_fset}",
            priority,
            [ant("Nitrogen", n_stat), ant("Potassium", k_stat), ant(env_var, env_fset)],
            cons,
        )
...
priority = 8
for t_fset, _ in temp_sets:
    for r_fset, _ in rain_sets:
        if t_fset == "hot" and r_fset == "low":
            rule(
                "Hot_Dry",
                "Hot+dry: stress mitigation",
                9,
                [ant("Temperature_Max", "hot"), ant("Rainfall", "low")],
                [
                    cons_nutrient("Nitrogen", "maintain"),
                    cons_nutrient("Potassium", "increase", 10, "%"),
                    n_adj("medium"),
                    p_adj("high"),
                    i_adj("high"),
                    d_adj("low"),
            ...
            rule(
                "Hot_Wet",
                "Hot+wet: disease risk",
                8,
                [ant("Temperature_Max", "hot"), ant("Rainfall", "high")],
                [
                    cons_nutrient("Nitrogen", "reduce", 10, "%"),
                    cons_nutrient("Phosphorus", "maintain"),
                    n_adj("low"),
                    d_adj("low"),
                    i_adj("low"),
            ...
            rule(
                "Moderate_MediumRain",
                "Moderate temp + medium rain: ideal",
                5,
...
for ph_fset, _ in pH_sets:
    for n_stat in ["low", "medium", "high"]:
        ...
        if ph_fset == "acidic" and n_stat == "low":
            ...
            cons = [
                cons_nutrient("Nitrogen", "reduce", 15, "%"),
                n_adj("low"),
                d_adj("low"),
                cons_risk("medium"),
                cons_conf("medium"),
        ...
        rule(
            rule_name,
            f"pH={ph_fset} + N={n_stat}",
            priority,
            [ant("Soil_pH", ph_fset), ant("Nitrogen", n_stat)],
            cons,
        )
...
for oc_fset in ["low", "medium", "high"]:
    for n_stat in ["low", "medium", "high"]:
        ...
        if oc_fset == "low" and n_stat == "low":
            cons = [
                cons_nutrient("Nitrogen", "increase", 30, "%"),
                n_adj("high"),
                d_adj("high"),
                cons_risk("high"),
                cons_conf("low"),
        ...
        rule(
            rule_name,
            f"OC={oc_fset} + N={n_stat}",
            priority,
            [ant("Organic_Carbon", oc_fset), ant("Nitrogen", n_stat)],
            cons,
        )
...
for z_fset in ["low", "medium", "high"]:
    for ph_fset in ["acidic", "neutral", "alkaline"]:
        ...
        rule(
            rule_name,
            f"Zinc={z_fset} + pH={ph_fset}",
            priority,
            [ant("Zinc", z_fset), ant("Soil_pH", ph_fset)],
            cons,
        )

# ── GROWTH STAGE RULES ──
priority = 7
for gs_fset, _ in growth_sets:
    for n_stat in ["low", "medium", "high"]:
        ...
        if gs_fset == "seedling" and n_stat == "low":
            ...
            cons = [
                cons_nutrient("Nitrogen", "increase", 20, "%"),
                cons_nutrient("Phosphorus", "increase", 10, "%"),
                n_adj("high"),
                d_adj("high"),
                cons_risk("high"),
                cons_conf("medium"),
            ...
            cons = [
                cons_nutrient("Nitrogen", "reduce", 20, "%"),
                n_adj("low"),
                d_adj("low"),
                cons_risk("low"),
                cons_conf("high"),
        ...
        rule(
            rule_name,
            f"Stage={gs_fset} + N={n_stat}",
            priority,
            [ant("Growth_Stage", gs_fset), ant("Nitrogen", n_stat)],
            cons,
        )
...
for y_fset in ["low", "medium", "high"]:
    for n_stat in ["low", "medium", "high"]:
        ...
        rule(
            rule_name,
            f"Yield={y_fset} + N={n_stat}",
            priority,
            [ant("Yield_Prediction", y_fset), ant("Nitrogen", n_stat)],
            cons,
        )
...
for n_stat, p_stat, k_stat in product(["low", "medium", "high"], repeat=3):
    ...
    risk = (
        "high"
        if sum(1 for s in [n_stat, p_stat, k_stat] if s == "low") >= 2
        else "medium"
        if any(s == "low" for s in [n_stat, p_stat, k_stat])
        else "low"
    )
    conf = "high" if all(s == "medium" for s in [n_stat, p_stat, k_stat]) else "medium"
    rule(
        rule_name,
        f"N={n_stat} P={p_stat} K={k_stat} 3-factor",
        priority,
        [ant("Nitrogen", n_stat), ant("Phosphorus", p_stat), ant("Potassium", k_stat)],
        [
            cons_nutrient("Nitrogen", n_act, n_amt, "%"),
            cons_nutrient("Phosphorus", p_act, p_amt, "%"),
...
for ph_fset in ["acidic", "neutral", "alkaline"]:
    for r_fset in ["low", "medium", "high"]:
        for t_fset in ["cool", "moderate", "hot"]:
            ...
            risk = (
                "high"
                if (t_fset == "hot" and r_fset == "low") or (t_fset == "cool" and r_fset == "low")
                else "medium"
                if (t_fset == "hot" or r_fset == "low" or r_fset == "high")
                else "low"
            )
            i_adj_val = "high" if r_fset == "low" else "low" if r_fset == "high" else "medium"
            d_adj_val = (
                "low"
                if t_fset == "hot" and r_fset == "high"
                else "high"
                if t_fset == "hot" and r_fset == "low"
                else "medium"
            )
            rule(
                rule_name,
                f"pH={ph_fset} Rain={r_fset} Temp={t_fset}",
                priority,
                ...
                [
                    ...
                    d_adj(d_adj_val),
                    cons_risk(risk),
                    cons_conf("medium" if risk != "low" else "high"),
                ],
            )
...
for n_stat in ["low", "medium", "high"]:
    rule(
        ...
        [
            cons_nutrient(
                "Nitrogen",
                "increase" if n_stat == "low" else "reduce" if n_stat == "high" else "maintain",
                20 if n_stat == "low" else 10 if n_stat == "high" else 0,
                "%",
            ),
            n_adj("high" if n_stat == "low" else "low" if n_stat == "high" else "medium"),
            d_adj("medium"),
            i_adj("medium"),
            cons_risk("low" if n_stat == "medium" else "medium"),
            cons_conf("high" if n_stat == "medium" else "medium"),
        ],
    )
...
for p_stat in ["low", "medium", "high"]:
    rule(
        ...
        f"P={p_stat} alone",
        priority,
        [ant("Phosphorus", p_stat)],
        [
            cons_nutrient(
                "Phosphorus",
                "increase" if p_stat == "low" else "reduce" if p_stat == "high" else "maintain",
                20 if p_stat == "low" else 10 if p_stat == "high" else 0,
                "%",
            ),
            n_adj("medium"),
            d_adj("medium"),
            i_adj("medium"),
            cons_risk("low" if p_stat == "medium" else "medium"),
            cons_conf("high" if p_stat == "medium" else "medium"),
        ],
    )
...
for k_stat in ["low", "medium", "high"]:
    rule(
        ...
        f"K={k_stat} alone",
        priority,
        [ant("Potassium", k_stat)],
        [
            cons_nutrient(
                "Potassium",
                "increase" if k_stat == "low" else "reduce" if k_stat == "high" else "maintain",
                20 if k_stat == "low" else 10 if k_stat == "high" else 0,
                "%",
            ),
            p_adj("high" if k_stat == "low" else "low" if k_stat == "high" else "medium"),
            d_adj("medium"),
            i_adj("medium"),
            cons_risk("low" if k_stat == "medium" else "medium"),
            cons_conf("high" if k_stat == "medium" else "medium"),
        ],
    )

for ph_fset in ["acidic", "neutral", "alkaline"]:
    rule(
        f"Single_pH_{ph_fset}",
        f"pH={ph_fset} alone",
        priority,
        [ant("Soil_pH", ph_fset)],
        [
            ...
            d_adj("medium"),
            i_adj("medium"),
            cons_risk("low" if ph_fset == "neutral" else "medium"),
            cons_conf("high" if ph_fset == "neutral" else "medium"),
        ],
    )

for r_fset in ["low", "medium", "high"]:
    rule(
        f"Single_Rain_{r_fset}",
        f"Rain={r_fset} alone",
        priority,
        [ant("Rainfall", r_fset)],
        [
            ...
            d_adj("medium"),
            cons_risk("low" if r_fset == "medium" else "medium"),
            cons_conf("high"),
        ],
    )
...
for t_fset in ["cool", "moderate", "hot"]:
    rule(
        f"Single_Temp_{t_fset}",
        f"Temp={t_fset} alone",
        priority,
        [ant("Temperature_Max", t_fset)],
        [
            cons_nutrient("Nitrogen", "maintain"),
            n_adj("medium"),
...
for z_fset in ["low", "medium", "high"]:
    rule(
        ...
        [
            ...
            cons_risk("medium" if z_fset == "low" else "low"),
            cons_conf("high" if z_fset != "low" else "medium"),
        ],
    )

for oc_fset in ["low", "medium", "high"]:
    rule(
        f"Single_OC_{oc_fset}",
        f"OC={oc_fset} alone",
        priority,
        [ant("Organic_Carbon", oc_fset)],
        [
            cons_nutrient(
                "Nitrogen",
                "increase" if oc_fset == "low" else "reduce" if oc_fset == "high" else "maintain",
                10 if oc_fset == "low" else 0,
                "%",
            ),
            n_adj("medium" if oc_fset == "low" else "low" if oc_fset == "high" else "medium"),
            d_adj("medium"),
            i_adj("medium"),
            cons_risk("low" if oc_fset != "low" else "medium"),
            cons_conf("high" if oc_fset != "low" else "medium"),
        ],
    )

for gs_fset in ["seedling", "vegetative", "flowering", "maturity"]:
    rule(
        f"Single_Stage_{gs_fset}",
        f"Stage={gs_fset} alone",
        priority,
        [ant("Growth_Stage", gs_fset)],
        [
            cons_nutrient("Nitrogen", "maintain"),
            n_adj("medium"),
            d_adj("medium"),
...
for y_fset in ["low", "medium", "high"]:
    rule(
        f"Single_Yield_{y_fset}",
        f"Yield={y_fset} alone",
        priority,
        [ant("Yield_Prediction", y_fset)],
        [
            cons_nutrient("Nitrogen", "maintain"),
            n_adj("medium"),
            d_adj("medium"),
            i_adj("medium"),
            cons_risk("low" if y_fset != "low" else "high"),
            cons_conf("high" if y_fset == "high" else "medium"),
        ],
    )
...
for gs_fset in ["seedling", "vegetative", "flowering", "maturity"]:
    for p_stat in ["low", "high"]:
        rule(
            f"Stage_{gs_fset}_P{p_stat}",
            f"Stage={gs_fset} + P={p_stat}",
            priority,
            [ant("Growth_Stage", gs_fset), ant("Phosphorus", p_stat)],
            [
                cons_nutrient(
                    "Phosphorus",
                    "increase" if p_stat == "low" else "reduce",
                    20 if p_stat == "low" else 10,
                    "%",
                ),
                n_adj("medium"),
                d_adj("medium"),
                i_adj("medium"),
                cons_risk("low" if gs_fset != "flowering" else "medium"),
                cons_conf("medium"),
            ],
        )
...
for gs_fset in ["seedling", "vegetative", "flowering", "maturity"]:
    for k_stat in ["low", "high"]:
        rule(
            f"Stage_{gs_fset}_K{k_stat}",
            f"Stage={gs_fset} + K={k_stat}",
            priority,
            [ant("Growth_Stage", gs_fset), ant("Potassium", k_stat)],
            [
                cons_nutrient(
                    "Potassium",
                    "increase" if k_stat == "low" else "reduce",
                    20 if k_stat == "low" else 10,
                    "%",
                ),
                p_adj("high" if k_stat == "low" else "low"),
                d_adj("medium"),
                i_adj("medium"),
                cons_risk("medium" if k_stat == "low" else "low"),
                cons_conf("medium"),
            ],
        )
...
for oc_fset in ["low", "high"]:
    for ph_fset in ["acidic", "alkaline"]:
        rule(
            f"OC_{oc_fset}_pH_{ph_fset}",
            f"OC={oc_fset} + pH={ph_fset}",
            priority,
            [ant("Organic_Carbon", oc_fset), ant("Soil_pH", ph_fset)],
            [
                cons_nutrient(
                    "Nitrogen",
                    "increase" if oc_fset == "low" else "maintain",
                    15 if oc_fset == "low" else 0,
                    "%",
                ),
                n_adj("high" if oc_fset == "low" else "medium"),
                d_adj("medium"),
                i_adj("medium"),
                cons_risk("high" if oc_fset == "low" and ph_fset == "acidic" else "medium"),
                cons_conf("medium"),
            ],
        )
...
for z_fset in ["low"]:
    for r_fset in ["low", "high"]:
        rule(
            f"Zinc_{z_fset}_Rain_{r_fset}",
            f"Zinc low + Rain={r_fset}",
            priority,
            [ant("Zinc", z_fset), ant("Rainfall", r_fset)],
            [
                cons_nutrient("Zinc", "apply_foliar_spray"),
                d_adj("medium"),
                i_adj("high" if r_fset == "low" else "low"),
                cons_risk("high" if r_fset == "low" else "medium"),
                cons_conf("medium"),
            ],
        )
...
rule(
    "All_Optimal_NP_Kmedium",
    "All N,P medium, K medium",
    3,
...
rule(
    "All_High_NPK",
    "All N,P,K high: reduce all",
    4,
    [ant("Nitrogen", "high"), ant("Phosphorus", "high"), ant("Potassium", "high")],
    [
        cons_nutrient("Nitrogen", "reduce", 20, "%"),
        cons_nutrient("Phosphorus", "reduce", 15, "%"),
        cons_nutrient("Potassium", "reduce", 15, "%"),
        n_adj("low"),
        p_adj("low"),
        i_adj("medium"),
        d_adj("low"),
...
rule(
    "All_Low_NPK",
    "All N,P,K low: emergency boost",
    10,
    [ant("Nitrogen", "low"), ant("Phosphorus", "low"), ant("Potassium", "low")],
    [
        cons_nutrient("Nitrogen", "increase", 30, "%"),
        cons_nutrient("Phosphorus", "increase", 25, "%"),
        cons_nutrient("Potassium", "increase", 25, "%"),
        n_adj("high"),
        p_adj("high"),
        i_adj("medium"),
