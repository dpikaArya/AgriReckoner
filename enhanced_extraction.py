...
CROP_MAP = {
    "bell pepper": "Bell Pepper", "capsicum annuum": "Bell Pepper",
    "black wheat": "Black Wheat", "triticum": "Black Wheat",
    "carrot": "Carrot", "daucus carota": "Carrot",
    "cowpea": "Cowpea", "vigna unguiculata": "Cowpea",
    "spinach": "Spinach", "spinacia oleracea": "Spinach",
    "chickpea": "Chickpea", "cicer arietinum": "Chickpea",
    "barley": "Barley", "hordeum": "Barley",
    "maize": "Maize", "zea mays": "Maize",
}
DESIGN_MAP = {
    'randomized block design': 'RBD', 'randomized complete block design': 'RCBD',
    'split plot': 'Split Plot', 'completely randomized design': 'CRD',
}
...
HEADER_MAP = [
    ("Yield_per_Plot", [r'yield.*plot', r'plot.*yield', r'total yield', r'fruit yield', r'fresh weight', r'^yield$', r'plot.*weight']),
    ...
    ("Biomass_Yield", [r'biomass yield', r'biological yield', r'straw yield', r'dry yield', r'total biomass']),
...
def extract_context(lines, full_text):
    ...
    if yrs: m['Year'] = max(set(yrs), key=yrs.count)
    for kw, vl in DESIGN_MAP.items():
        if re.search(kw, full_text, re.IGNORECASE): m['Design'] = vl; break
    for pat, key in [
...
def yield_regex_extraction(full_text):
    ...
    results = {}
    pats = [
        (r'(?:grain\s*yield|seed\s*yield|grain\s*yield|biological\s*yield)\s*(?:of|was|is|:|=)\s*(\d+\.?\d*)\s*(?:kg\s*ha-1|t\s*ha-1|kg/ha|t/ha|q/ha)', 'Yield_per_Hectare'),
        (r'(\d+\.?\d*)\s*(?:kg\s*ha-1|t\s*ha-1)', 'Yield_per_Hectare'),
        (r'(?:recorded|observed|maximum|highest|average)\s*(?:grain\s*yield|seed\s*yield)\s*(?:of|was|is)?\s*(\d+\.?\d*)', 'Yield_per_Hectare'),
        (r'(?:yield|grain\s*yield)\s*(?:was|ranged|varied)\s*(?:from)?\s*(\d+\.?\d*)', 'Yield_per_Hectare'),
    ]
    for pat, var in pats:
            ...
            if consec:
                ...
                while pos < min(block[-1][0] + 180, len(lines)):
                    ...
                    if not line:
                        ...
                        continue
                    empty_streak = 0
                    if line.startswith('Figure') or line.startswith('Fig.'): break
                    if re.match(r'^#', line): continue

                    # Check for new treatment block
                    sl = line.rstrip('*†‡†‡').strip()
                    if sl in tid_pos_set:
                        pos -= 1; break  # Put it back
                    ...
                    if re.search(r'[a-zA-Z]{3,}', line):
                        ...
                        for _ in range(len(tids)):
                            ...
                            if nxt.rstrip('*†‡').strip() in tid_pos_set: break
                            if re.search(r'[a-zA-Z]{3,}', nxt) and not re.search(r'\d', nxt) and len(nxt) > 3:
                                break
                            if re.search(r'\d', nxt):
                                values.append(nxt); pos += 1
                            else: break
                        if values:
...
def process_one_pdf(pdf_path):
    ...
    if not any(row.get('Yield_per_Hectare') or row.get('Yield_per_Plot') or row.get('Biomass_Yield')
              for row in all_rows):
...
if __name__ == '__main__':
    ...
    for yc in ['Yield_per_Hectare', 'Yield_per_Plot', 'Yield_per_Acre', 'Biomass_Yield', 'Harvest_Index']:
        if yc in df.columns:
            n = df[yc].notna().sum()
            print(f"  {yc}: {n} non-null ({n/len(df)*100:.1f}%)")

    print(f"\n  Missing % for target variables:")
    for var in ['Yield_per_Hectare', 'Yield_per_Plot', 'Plant_Height_cm', 'SPAD',
                'Soil_pH', 'Nitrogen', 'Phosphorus', 'Potassium',
                'Organic_Carbon', 'Protein']:
