...
def export_narrative_report():
    ...
    if val is not None:
        ...
        cols = [
            c
            for c in ["Column", "Non_Null", "Missing", "Missing_Pct", "Issues"]
            if c in val.columns
        ]
...
def main():
    ...
    log(f"  2. {docx1.name}")
    log("     Full narrative report (8 sections)")
    log(f"  3. {docx2.name}")
    log("     Regression analysis report (6 sections)")
