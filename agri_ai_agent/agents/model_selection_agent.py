...
EXCLUDE_COLS = (
    ...
    | NON_FEATURE_COLS
    | frozenset(POST_HARVEST_VARIABLES)
)
...
class ModelSelectionAgent(BaseAgent):
    ...
    def process(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        scoring = kwargs.get(
            "scoring", "neg_root_mean_squared_error" if not is_classification else "accuracy"
        )
        ...
        if X is None or len(X) < 10:
            self.log.warning(
                "Insufficient data for model selection (%d samples)", len(X) if X is not None else 0
            )
            return df

        n_samples = len(X)
        self.log.info(
            "Model selection: %d samples, %d features, target='%s'",
            n_samples,
            X.shape[1],
            target_col,
        )
    ...
    def _evaluate_model(
        ...
        try:
            ...
            result.update(
                self._tune_in_sample(model, param_grid, X, y, outer_cv, scoring, optimize)
            )
            self._add_generalization_scores(
                result, model, param_grid, X, y, outer_cv, n_splits, scoring, optimize
            )
    ...
    def _build_inner_search(self, model, param_grid, scoring):
        ...
        return GridSearchCV(
            pipeline, grid, cv=inner_cv, scoring=scoring, n_jobs=-1, refit=True, error_score=np.nan
        )
    ...
    def _score_estimator(self, estimator, X, y, cv):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            scores = cross_validate(
                estimator, X, y, cv=cv, n_jobs=-1, scoring=("r2", "neg_mean_squared_error")
            )
        return {
    ...
    def _nesting_skip_reason(self, n_samples, outer_splits, optimize, param_grid):
        ...
        return (
            f"n={n_samples} too small for {outer_splits}x{INNER_CV_FOLDS} "
    ...
    def _log_evaluation(self, name, result):
        ...
        self.log.info(
            "  %s: cv_score=%.4f, R2=%.4f±%.4f, RMSE=%.4f (nested=%s)",
            name,
            result["cv_score"],
            result.get("r2_mean", 0),
            result.get("r2_std", 0),
            result.get("rmse_mean", 0),
            result.get("nested_cv", False),
        )
        ...
        for r in cv_results:
            ...
            try:
                if hasattr(model, "feature_importances_"):
                    ...
                    for feat, val in zip(feature_names, vals, strict=True):
                        all_imp.append(
                            {
                                "model": name,
                                "feature": feat,
    ...
    def _build_report(
        ...
        lines = (
            header
            + self._honesty_note(cv_results, advisory)
            + [
                "",
                "## Leaderboard",
                "",
