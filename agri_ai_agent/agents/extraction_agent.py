...
RANGE_CONSTRAINTS = {
    "soil_ph": (3.0, 10.0),
    "ec_ds_m": (0.0, 10.0),
    "organic_carbon_pct": (0.0, 10.0),
    "nitrogen_kg_ha": (0.0, 1000.0),
    "phosphorus_kg_ha": (0.0, 500.0),
    "potassium_kg_ha": (0.0, 1000.0),
    "temperature_max_c": (-20.0, 55.0),
    "temperature_min_c": (-30.0, 40.0),
    "rainfall_mm": (0.0, 10000.0),
    "humidity_pct": (0.0, 100.0),
    "yield_kg_ha": (0.0, 50000.0),
    "plant_height_cm": (0.0, 500.0),
    "spad": (0.0, 80.0),
    "protein_pct": (0.0, 60.0),
    "fruit_weight_g": (0.0, 5000.0),
    "fruit_number": (0.0, 10000.0),
    "harvest_index": (0.0, 1.5),
    "100_seed_weight_g": (0.0, 500.0),
}

ONTOLOGY_KNOWLEDGE_BASE = {
    "Crop": {
        "AGROVOC": "http://aims.fao.org/aos/agrovoc/c_2556",
        "CropOntology": "CO_320:0000000",
        "FoodOn": "FOODON:00001002",
        "label": "Crop",
        "synonyms": ["crop type", "cultivated plant", "crop species"],
    ...
    "Soil_pH": {
        ...
        "label": "Soil pH",
        "synonyms": ["pH", "soil acidity", "soil reaction", "pH value"],
        "parent": "Soil property",
        "children": [],
    },
    "EC": {
    ...
    "Nitrogen": {
        ...
        "CropOntology": "CO_320:0000020",
        "FoodOn": "FOODON:00001310",
        "label": "Nitrogen",
        "synonyms": ["N", "available nitrogen", "total nitrogen", "soil nitrogen"],
        "parent": "Plant nutrient",
        "children": ["Ammonium nitrogen", "Nitrate nitrogen"],
    },
    "Phosphorus": {
        "AGROVOC": "http://aims.fao.org/aos/agrovoc/c_5801",
        "label": "Phosphorus",
        "synonyms": ["P", "phosphorous", "available phosphorus", "phosphate"],
        "parent": "Plant nutrient",
        "children": [],
    },
    "Potassium": {
        ...
        "label": "Potassium",
        "synonyms": ["K", "available potassium", "potash"],
        "parent": "Plant nutrient",
        "children": [],
    },
    "Temperature_Max": {
...
ADDITIONAL_SYNONYMS = {
    ...
    "zaid": "Season",
    "sowing_date": "Season",
    "planting_date": "Season",
    "harvest_date": "Growth_Duration_Days",
    "days_to_harvest": "Growth_Duration_Days",
    "crop_duration": "Growth_Duration_Days",
    "maturity_days": "Growth_Duration_Days",
    "no_of_replications": "Replications",
    "rep": "Replications",
    "repititions": "Replications",
    "plot_area_m2": "Plot_Size",
    "plot_dimension": "Plot_Size",
    "row_to_row": "Spacing_Row",
    "plant_to_plant": "Spacing_Plant",
    "intra_row": "Spacing_Plant",
    "inter_row": "Spacing_Row",
    "biofertilizers": "Biofertilizer",
    "bio_fertilizers": "Biofertilizer",
    "inoculation": "Biofertilizer",
    "organic_amendments": "Organic_Fertilizer",
    "farmyard_manure": "Organic_Fertilizer",
    "fym": "Organic_Fertilizer",
    "compost": "Organic_Fertilizer",
    "vermicompost": "Organic_Fertilizer",
    "n_dose": "Dose",
    "p_dose": "Dose",
    "k_dose": "Dose",
    "fertilizer_dose": "Dose",
    "n_kg_ha": "Nitrogen",
    "p_kg_ha": "Phosphorus",
    "k_kg_ha": "Potassium",
    "available_n_kg_ha": "Nitrogen",
    "available_p_kg_ha": "Phosphorus",
    "available_k_kg_ha": "Potassium",
    "total_n": "Nitrogen",
    "total_p": "Phosphorus",
    "total_k": "Potassium",
    "plant_height_at_harvest": "Plant_Height_cm",
    "final_plant_height": "Plant_Height_cm",
    "shoot_length_at_harvest": "Shoot_Length_cm",
    "root_length_at_harvest": "Root_Length_cm",
    "leaf_area_index": "Leaf_Area_cm2",
    "lai": "Leaf_Area_cm2",
    "chlorophyll_spad": "SPAD",
    "chlorophyll_content_spad": "SPAD",
    "number_of_branches": "Branches",
    "branches_per_plant": "Branches",
    "number_of_flowers": "Flowers",
    "flowers_per_plant": "Flowers",
    "number_of_nodes": "Nodes",
    "seed_yield": "Yield_per_Plot",
    "grain_yield": "Yield_per_Plot",
    "economic_yield": "Yield_per_Plot",
    "biological_yield": "Biomass_Yield",
    "total_biomass": "Biomass_Yield",
    "straw_yield": "Biomass_Yield",
    "test_weight_g": "100_Seed_Weight",
    "thousand_grain_weight": "100_Seed_Weight",
...
class ExtractionAgent(BaseAgent):
    ...
    def process(self, df: pd.DataFrame | None = None, **kwargs) -> pd.DataFrame:
        ...
        if df is not None and not df.empty:
            self.dataframe = df
            self.log.info(
                "[%s] Using provided DataFrame with %d columns", self.agent_name, len(df.columns)
            )
        elif filepath:
        ...
        if self.dataframe is not None and not self.dataframe.empty:
            ...
            var_names = list(
                {
                    e.get("canonical_variable", e.get("variable", ""))
        ...
        self._save_artifacts(
            ingestion_result, document_structures, extracted_entities, ontology_mappings
        )
    ...
    def _ingest(self, filepath: str, sheet_name: str | None = None) -> tuple[pd.DataFrame, dict]:
        fp = Path(filepath)
        if not fp.exists():
    ...
    def _extract_from_papers(self, papers_dir: str) -> list[dict]:
        ...
        for pdf_path in pdf_files:
            ...
            self.log.info(
                "[%s] Extracted %d entities from %s", self.agent_name, len(entities), pdf_path.name
            )
    ...
    def _parse_document(self, pdf_path: Path) -> dict:
        ...
        title = ""
        for _, line in enumerate(lines[:5]):
            s = line.strip()
            if len(s) > 20:
                title = s
                break
    ...
    def _extract_text(self, pdf_path: Path) -> str:
        ...
        try:
            ...
            self.log.warning(
                "[%s] pdftotext extraction failed for %s: %s", self.agent_name, pdf_path.name, e
            )
        return ""
    ...
    def _ner_from_text(self, text: str, source: str = "text") -> list[dict]:
        ...
        patterns = [
            ...
            (
                r"(?:protein|protein\s*content)\s*[:=]?\s*([\d.]+)\s*%",
                "Quality_Trait",
                "Protein_pct",
            ),
    ...
    def _map_to_ontology(self, variables) -> list[dict]:
        ...
        mapped = sum(
            1
            for r in rows
            if r["AGROVOC_ID"] or r["CropOntology_ID"] or r["ENVO_ID"] or r["FoodOn_ID"]
        )
    ...
    def _map_to_schema(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing_uams:
            result_df = pd.concat(
                [
                    result_df,
                    pd.DataFrame({col: pd.NA for col in missing_uams}, index=result_df.index),
                ],
                axis=1,
            )
        ...
        if unmapped:
            self.log.warning(
                "[%s] %d columns unmapped: %s", self.agent_name, len(unmapped), unmapped[:10]
            )

        self.log.info(
            "[%s] Schema mapped: %d cols → %d UAMS cols",
            self.agent_name,
            len(df.columns),
            len(ordered),
        )
    ...
    def _save_artifacts(
        ...
        if ingestion:
            path = self.save_text_artifact(json.dumps(ingestion, indent=2), "ingestion_report.json")
            self.log.info("[%s] Ingestion report → %s", self.agent_name, path)

        if entities:
            path = self.save_text_artifact(
                json.dumps(entities, indent=2, default=str), "extracted_entities.json"
            )
