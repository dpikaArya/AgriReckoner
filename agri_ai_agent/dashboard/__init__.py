...
class EvaluationDashboard:
    ...
    def compute_extraction_accuracy(
        self, extracted_df: pd.DataFrame, reference_df: pd.DataFrame | None = None
    ) -> float:
    ...
    def compute_schema_mapping_accuracy(self, df: pd.DataFrame, schema_columns: list[str]) -> float:
    ...
    def compute_missing_data_rate(self, df: pd.DataFrame) -> float:
        ...
        self._details["missing_data_rate"] = {
            ...
            "value": round(rate, 4),
            "total_cells": int(
                df.select_dtypes(include=[np.number]).shape[0]
    ...
    def compute_validation_pass_rate(
        self, df: pd.DataFrame, validation_issues: list | None = None
    ) -> float:
        if validation_issues is None:
            validation_issues = []
        total_rows = len(df)
        issue_rows = len(
            set(issue.get("row", 0) for issue in validation_issues if isinstance(issue, dict))
        )
    ...
    def compute_feature_coverage(
        self, original_df: pd.DataFrame, engineered_df: pd.DataFrame
    ) -> float:
    ...
    def compute_all(
        ...
        if original_df is not None:
            ...
            self._details["feature_coverage"] = {
                "description": "No original df provided",
                "value": 0.0,
            }
        ...
        if agent_results is not None:
            ...
            self._metrics["pipeline_success_rate"] = 0.0
            self._details["pipeline_success_rate"] = {
                "description": "No agent results provided",
                "value": 0.0,
            }
