...
def to_elemental(value, label):
    ...
    if basis == AMBIGUOUS:
        return (
            value,
            False,
            f"{label!r} names a form whose basis is not fixed; converting is a guess",
        )
...
_RECOMMENDATION_RATIO = re.compile(
    r"(\d+(?:\.\d+)?)\s*[:\-]\s*(\d+(?:\.\d+)?)\s*[:\-]\s*(\d+(?:\.\d+)?)"
)


def parse_recommendation_ratio(text):
    """Parse an N:P2O5:K2O (or N:P:K) ratio string into elemental-component dict.

    Handles formats like:
        - "120:60:60" (kg/ha implicit, oxide basis for P and K)
        - "120:60:60 kg/ha"
        - "N120:P60:K60"
        - "120-60-60"

    Returns a dict with keys "N", "P", "K" mapped to the *elemental* values,
    or None if the text does not contain a recognisable 3-part ratio.
    P and K values are converted from oxide to elemental basis (P2O5 -> P, K2O -> K).
    """
    if not text:
        return None
    match = _RECOMMENDATION_RATIO.search(str(text))
    if not match:
        return None
    n_val, p_val, k_val = (float(match.group(i)) for i in (1, 2, 3))
    p_factor = oxide_factor("P2O5")
    k_factor = oxide_factor("K2O")
    return {"N": n_val, "P": p_val * p_factor, "K": k_val * k_factor}
    ...
    for label, column in labels_to_columns.items():
        ...
        if basis == OXIDE:
            ...
            plan[label] = (
                KEEP,
                1.0,
                f"read on the schema basis, elemental ({formula or 'unstated'})",
            )
    return plan
