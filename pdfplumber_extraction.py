...
UAMS_MAP = {
    ...
    "plant length": "Plant_Height_cm",
    "shoot length": "Plant_Height_cm",
    "shoot": "Plant_Height_cm",
    "root length": "Plant_Height_cm",
    "root": "Plant_Height_cm",
    "height": "Plant_Height_cm",
    "tillers": "Tillers",
    "tiller": "Tillers",
    "productive tillers": "Tillers",
    "tiller number": "Tillers",
    "spike length": "Spike_Length",
    "ear length": "Spike_Length",
    "panicle length": "Spike_Length",
    "spike number": "Spike_Length",
    "spikenumber": "Spike_Length",
    "seeds per spike": "Seeds_per_Spike",
    "grains per spike": "Seeds_per_Spike",
    "grain per spike": "Seeds_per_Spike",
    "grains per ear": "Seeds_per_Spike",
    "100 seed weight": "100_Seed_Weight",
    "1000 grain weight": "100_Seed_Weight",
    "1000-grain weight": "100_Seed_Weight",
    "test weight": "100_Seed_Weight",
    "seed index": "100_Seed_Weight",
    "grain weight": "100_Seed_Weight",
    "seed weight": "100_Seed_Weight",
    "grain yield": "Yield_per_Hectare",
    "grainyield": "Yield_per_Hectare",
    "plot yield": "Yield_per_Plot",
    "yield per plot": "Yield_per_Plot",
    "yield per hectare": "Yield_per_Hectare",
    "yield t ha": "Yield_per_Hectare",
    "t ha": "Yield_per_Hectare",
    "yield per acre": "Yield_per_Acre",
    "q per acre": "Yield_per_Acre",
    "biomass": "Biomass_Yield",
    "fresh weight": "Biomass_Yield",
    "dry weight": "Biomass_Yield",
    "harvest index": "Harvest_Index",
    "fruit weight": "Fruit_Weight",
    "fruit diameter": "Fruit_Diameter_mm",
    "fruit length": "Fruit_Length",
    "root diameter": "Fruit_Diameter_mm",
    "leaf area": "Leaf_Area_cm2",
    "leaf area cm2": "Leaf_Area_cm2",
    "nitrogen": "Nitrogen",
...
def detect_treatment_column(header_row):
    for ci, cell in enumerate(header_row):
        ...
        if c in ("treatment", "treatments", "trt", "treatment code"):
            return ci
        if (
            TREATMENT_RE.match(cell.strip().rstrip("*†‡").strip())
                ...
                for table in tables:
                    ...
                    for ri in range(1, len(table)):
                        ...
                        if not is_treatment_row(row, treat_col):
                            continue
                        t_val = (
                            str(row[treat_col]).strip().rstrip("*†‡").strip()
                            if row[treat_col]
                            else ""
                        )
                        if not t_val:
                    ...
                    for ri in range(len(table)):
                        ...
                        for ci, cell in enumerate(row):
                            ...
                            if cell is None or str(cell).strip() in ("-", "", "–"):
                                continue
                            header_text = (
                                str(table[0][ci]).strip()
                                if table[0] and ci < len(table[0]) and table[0][ci]
                                else ""
                            )
                            ...
                            if "(" in header_text and ")" in header_text:
                                unit_part = header_text[
                                    header_text.index("(") : header_text.index(")") + 1
                                ]
                                clean_header = (
                                    header_text[: header_text.index("(")].strip() + " " + unit_part
                                )
                            else:
...
def process_pdf(pdf_path):
    ...
    for sr in source_rows:
        seen_treatments.add(
            sr.get("Treatment", "") + str(sr.get("Page", "")) + str(sr.get("Table_Row", ""))
        )
    for r in fpls_rows:
...
def main():
    ...
    cols = ["Source_File", "Treatment"] + sorted(
        [c for c in df.columns if c not in ("Source_File", "Treatment")]
    )
