...
model_df = (
    pd.read_excel(model_metrics_xlsx, engine="openpyxl")
    if model_metrics_xlsx.exists()
    else pd.DataFrame()
)
rec_df = pd.read_csv(recommendations_csv) if recommendations_csv.exists() else pd.DataFrame()
rr_df = (
    pd.read_excel(ready_reckoner_xlsx, engine="openpyxl")
    if ready_reckoner_xlsx.exists()
    else pd.DataFrame()
)
...
from agri_ai_agent.config.schema import SCHEMA_GROUPS, UAMS_COLUMNS  # noqa: E402
...
n_new = (
    int((ingestion_df["Status"] == "New").sum())
    if not ingestion_df.empty and "Status" in ingestion_df.columns
    else 0
)
n_prev = (
    int((ingestion_df["Status"] == "Previously Processed").sum())
    if not ingestion_df.empty and "Status" in ingestion_df.columns
    else 0
)
n_dups = (
    int((ingestion_df["Duplicate_Flag"] == "Yes").sum())
    if not ingestion_df.empty and "Duplicate_Flag" in ingestion_df.columns
    else 0
)
...
# Columns with data
cols_with_data = (
    sum(1 for c in UAMS_COLUMNS if c in schema_df.columns and schema_df[c].notna().any())
...
if validation_xlsx.exists():
    try:
        ...
        val_issues = (
            len(val_df[val_df["Missing_Pct"] > 0]) if "Missing_Pct" in val_df.columns else 0
        )
    except Exception:
        pass
...
n_targets = (
    model_df["Target"].nunique() if not model_df.empty and "Target" in model_df.columns else 0
)
n_best_r2 = 0
...
best_r2_str = (
    f"{model_df['R2'].max():.4f}" if (not model_df.empty and "R2" in model_df.columns) else "N/A"
)
...
layers = [
    ...
    ("Phase 7", "Fuzzy Logic", f"Rules={n_fuzzy_rules}, Input Vars=10", f"{fuzzy_eff}%"),
    (
        "Phase 8",
    ...
    ("Phase 9", "Ready Reckoner", f"Entries={n_reckoner}", f"{reckoner_eff}%"),
    ("Phase 10", "Continuous Learning", f"Registered={n_registered}", f"{cl_eff}%"),
]
...
if not model_df.empty:
    for _, row in model_df.iterrows():
        ...
        print(
            f"  {model_name:20s} | Target: {target:20s} | R²={r2:.4f} ({r2_pct}%) | RMSE={rmse:.4f} | MAE={mae:.4f} | MAPE={mape:.2f}% | CV_R²={cv_r2:.4f} ({cv_pct}%)"
        )
...
print(
    f"  Schema groups covered:         {len([g for g, cols in SCHEMA_GROUPS.items() if any(c in schema_df.columns for c in cols)])}/{len(SCHEMA_GROUPS)}"
)
...
for phase, name, _, eff in layers:
...
print("=" * 80)
print("  1. outputs/Ready_Reckoner_Table_Updated.csv")
print("  2. outputs/Ready_Reckoner_Updated.xlsx")
print("  3. outputs/AAIF_Layer_Efficiency_Report.txt")
print("  4. outputs/AAIF_Final_Report.html (pipeline report)")
print("  5. outputs/model_metrics.xlsx (model details)")
print("=" * 80)
