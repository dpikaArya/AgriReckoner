...
def test_the_basis_is_read_from_the_unit_when_the_term_names_the_element():
    ...
    assert _elemental_values(pattern, "phosphorus 60 kg P2O5 ha-1") == [
        pytest.approx(26.1856, rel=1e-4)
    ]
...
# --- Phase 6: Recommendation ratio parser ---


def test_parse_recommendation_ratio_basic():
    from agri_ai_agent.extractors.nutrients import parse_recommendation_ratio

    result = parse_recommendation_ratio("120:60:60")
    assert result is not None
    assert result["N"] == 120.0
    assert result["P"] == pytest.approx(60.0 * oxide_factor("P2O5"), rel=1e-4)
    assert result["K"] == pytest.approx(60.0 * oxide_factor("K2O"), rel=1e-4)


def test_parse_recommendation_ratio_with_unit():
    from agri_ai_agent.extractors.nutrients import parse_recommendation_ratio

    result = parse_recommendation_ratio("100:50:50 kg/ha")
    assert result is not None
    assert result["N"] == 100.0
    assert result["P"] == pytest.approx(50.0 * oxide_factor("P2O5"), rel=1e-4)
    assert result["K"] == pytest.approx(50.0 * oxide_factor("K2O"), rel=1e-4)


def test_parse_recommendation_ratio_with_dash_separator():
    from agri_ai_agent.extractors.nutrients import parse_recommendation_ratio

    result = parse_recommendation_ratio("80-40-40")
    assert result is not None
    assert result["N"] == 80.0
    assert result["P"] == pytest.approx(40.0 * oxide_factor("P2O5"), rel=1e-4)
    assert result["K"] == pytest.approx(40.0 * oxide_factor("K2O"), rel=1e-4)


def test_parse_recommendation_ratio_decimal_values():
    from agri_ai_agent.extractors.nutrients import parse_recommendation_ratio

    result = parse_recommendation_ratio("120.5:60.3:45.7")
    assert result is not None
    assert result["N"] == 120.5
    assert result["P"] == pytest.approx(60.3 * oxide_factor("P2O5"), rel=1e-4)
    assert result["K"] == pytest.approx(45.7 * oxide_factor("K2O"), rel=1e-4)


def test_parse_recommendation_ratio_returns_none_for_no_match():
    from agri_ai_agent.extractors.nutrients import parse_recommendation_ratio

    assert parse_recommendation_ratio("no ratio here") is None
    assert parse_recommendation_ratio("") is None
    assert parse_recommendation_ratio(None) is None


def test_parse_recommendation_ratio_only_two_parts():
    from agri_ai_agent.extractors.nutrients import parse_recommendation_ratio

    assert parse_recommendation_ratio("120:60") is None


def test_parse_recommendation_ratio_elemental_values_are_correct():
    """P and K values must be elemental, not oxide."""
    from agri_ai_agent.extractors.nutrients import parse_recommendation_ratio

    result = parse_recommendation_ratio("120:60:60")
    p_factor = oxide_factor("P2O5")
    k_factor = oxide_factor("K2O")
    assert result["P"] == pytest.approx(60.0 * p_factor, rel=1e-4)
    assert result["K"] == pytest.approx(60.0 * k_factor, rel=1e-4)
    assert result["P"] < 60.0, "P must be less than the oxide value"
    assert result["K"] < 60.0, "K must be less than the oxide value"


# --- Phase 3: Oxide guard in fuzzy matching ---


def test_map_column_oxide_formula_resolves_via_guard_not_fuzzy():
    """P2O5 in a column name must map to Phosphorus, not to Polyphenol_Oxidase via fuzzy."""
    from agri_ai_agent.external_data.column_mapper import map_column

    assert map_column("Generic", "P2O5_kg_ha") == "Phosphorus"
    assert map_column("Generic", "K2O_kg_ha") == "Potassium"
    assert map_column("Generic", "CaO_mg_kg") == "Calcium"


def test_map_column_oxide_formula_not_fuzzy_matched_to_unrelated():
    """An oxide formula must never fuzzy-match to Polyphenol_Oxidase or similar."""
    from agri_ai_agent.external_data.column_mapper import map_column

    result_p = map_column("Generic", "P2O5")
    result_k = map_column("Generic", "K2O")
    assert result_p == "Phosphorus"
    assert result_k == "Potassium"


def test_map_column_non_oxide_unrelated_columns_not_affected():
    """Non-oxide columns must still work as before."""
    from agri_ai_agent.external_data.column_mapper import map_column

    result = map_column("Generic", "Random_Junk_XYZ")
