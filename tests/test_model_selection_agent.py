...
from agri_ai_agent.agents.model_selection_agent import (
    ModelSelectionAgent, _get_model_pool,
)
...
class TestModelSelectionAgent:
    ...
    def test_optimized_vs_default(self, agent, small_df):
        r_opt = agent.process(small_df.copy(), target="Target_Yield",
                              optimize=True, cv_folds=3)
        r_def = agent.process(small_df.copy(), target="Target_Yield",
                              optimize=False, cv_folds=3)
    ...
    def test_custom_scoring(self, agent, small_df):
        result = agent.process(
            small_df, target="Target_Yield",
            scoring="r2", cv_folds=3,
        )
        assert "_model_selection_score" in result.columns
