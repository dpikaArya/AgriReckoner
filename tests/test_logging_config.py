...
def test_get_logger_returns_logger(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.utils.logging_config._PIPELINE_EXECUTION_LOG", tmp_path / "pipeline_execution.log"
    )
...
def test_logging_dir_created(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.utils.logging_config._PIPELINE_EXECUTION_LOG", log_dir / "pipeline_execution.log"
    )
