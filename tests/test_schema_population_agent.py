...
class TestSchemaPopulationAgent:
    ...
    def test_organic_matter_from_carbon(self):
        ...
        np.testing.assert_array_almost_equal(
            result["Organic_Matter"].values, expected, decimal=3
        )
    ...
    def test_organic_carbon_from_matter(self):
        ...
        np.testing.assert_array_almost_equal(
            result["Organic_Carbon"].values, expected, decimal=4
        )
    ...
    def test_biomass_from_shoot_root(self):
        ...
        expected = [30.0, 37.0]
        np.testing.assert_array_almost_equal(
            result["Biomass_Yield"].values, expected, decimal=2
        )
    ...
    def test_nue_calculation(self):
        df = pd.DataFrame({
            "Yield_per_Hectare": [4000.0, 5000.0],
            "Nitrogen": [120.0, 150.0],
        })
        result = self.agent.process(df)
        assert "Nitrogen_Use_Efficiency" in result.columns
        expected = [4000 / 120, 5000 / 150]
        np.testing.assert_array_almost_equal(
            result["Nitrogen_Use_Efficiency"].values, expected, decimal=2
        )
    ...
    def test_nx_p_interaction(self):
        ...
        expected = [4000, 9000]
        np.testing.assert_array_almost_equal(
            result["N_x_P"].values, expected, decimal=2
        )
