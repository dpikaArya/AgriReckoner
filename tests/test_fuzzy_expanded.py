...
from agri_ai_agent.rules.membership_functions import fuzzify, triangle, trapezoid, shouldered_s, shouldered_z


RULES_PATH = Path(__file__).resolve().parent.parent / "agri_ai_agent" / "rules" / "fertilizer_rules.yaml"
...
class TestFuzzyAgentExpanded:
    ...
    def test_fuzzy_output_columns(self, agent, sample_df):
        result = agent.process(sample_df)
        for col in ["Fuzzy_N_Action", "Fuzzy_P_Action", "Fuzzy_K_Action",
                     "Fuzzy_Zinc_Action", "Fuzzy_Risk", "Fuzzy_Confidence_Label", "Fuzzy_Summary"]:
    ...
    def test_n_action_labels(self, agent, sample_df):
        result = agent.process(sample_df)
        valid = {"Increase", "Reduce", "Maintain", "Apply foliar spray",
                 "Apply to soil", "Monitor", "Review program"}
    ...
    def test_different_inputs_different_outputs(self, agent):
        df1 = pd.DataFrame({"Nitrogen": [20], "Rainfall": [200], "Temperature_Max": [38], "Organic_Carbon": [0.3]})
        df2 = pd.DataFrame({"Nitrogen": [150], "Rainfall": [1200], "Temperature_Max": [20], "Organic_Carbon": [1.2]})
        r1 = agent.process(df1)
        r2 = agent.process(df2)
        assert r1.loc[0, "Fuzzy_Risk"] != r2.loc[0, "Fuzzy_Risk"] or r1.loc[0, "Fuzzy_N_Action"] != r2.loc[0, "Fuzzy_N_Action"]
    ...
    def test_dose_modified_by_fuzzy(self, agent, sample_df):
        result = agent.process(sample_df)
        assert "Recommended_Application_Interval" in result.columns or "Recommended_Fertilizer" in result.columns
