...
class TestLayer1Extraction:
    ...
    def test_extraction_agent_processes_csv(self):
        ...
        try:
            ...
            r = LayerResult(
                layer=self.LAYER, agent="ExtractionAgent",
                rows_in=rows_in, rows_out=len(result_df),
                cols_in=0, cols_out=len(result_df.columns),
            ...
            r = LayerResult(
                layer=self.LAYER, agent="ExtractionAgent",
                rows_in=rows_in, rows_out=0, cols_in=0, cols_out=0,
    ...
    def test_evidence_fusion_handles_empty(self):
        ...
        r = LayerResult(
            layer=self.LAYER, agent="EvidenceFusionAgent",
            rows_in=10, rows_out=len(agent.dataframe),
            cols_in=len(df.columns), cols_out=len(agent.dataframe.columns),
            elapsed_sec=round(elapsed, 3),
            passed=True,
        )
...
class TestLayer2Normalization:
    ...
    def test_ontology_agent_column_mapping(self):
        ...
        df = df.rename(columns={"Soil_pH": "ph", "Nitrogen": "n_total", "Rainfall": "precipitation"})
        t0 = time.time()
        contract = agent.run(df)
        elapsed = time.time() - t0
        result_df = agent.dataframe
        ...
        r = LayerResult(
            layer=self.LAYER, agent="OntologyAgent",
            rows_in=100, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            elapsed_sec=round(elapsed, 3),
    ...
    def test_table_intelligence_agent(self):
        ...
        assert contract.status == "success"
        r = LayerResult(
            layer=self.LAYER, agent="TableIntelligenceAgent",
            rows_in=100, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            elapsed_sec=round(elapsed, 3),
            throughput_rps=round(100 / max(elapsed, 0.001), 1),
    ...
    def test_schema_population_derives_safely(self):
        ...
        r = LayerResult(
            layer=self.LAYER, agent="SchemaPopulationAgent",
            rows_in=100, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            leakage_detected=0,
...
class TestLayer3Knowledge:
    ...
    def test_knowledge_agent_annotations(self):
        ...
        r = LayerResult(
            layer=self.LAYER, agent="KnowledgeAgent",
            rows_in=100, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            elapsed_sec=round(elapsed, 3),
            throughput_rps=round(100 / max(elapsed, 0.001), 1),
    ...
    def test_knowledge_integration_deterministic(self):
        ...
        result_df = agent.dataframe
        assert contract.status == "success"
        filled = result_df[["Soil_pH", "Organic_Carbon", "Nitrogen", "Phosphorus", "Potassium"]].notna().all(axis=1).sum()
        r = LayerResult(
            layer=self.LAYER, agent="KnowledgeIntegrationAgent",
            rows_in=50, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            elapsed_sec=round(elapsed, 3),
...
class TestLayer4Validation:
    ...
    def test_validation_agent_quality_checks(self):
        ...
        r = LayerResult(
            layer=self.LAYER, agent="ValidationAgent",
            rows_in=100, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            elapsed_sec=round(elapsed, 3),
            throughput_rps=round(100 / max(elapsed, 0.001), 1),
...
class TestLayer5ML:
    ...
    def test_feature_agent_no_yield_leakage(self):
        ...
        assert contract.status == "success"
        leaky_names = [
            "Yield_x_N", "Yield_x_pH", "Yield_x_Biomass", "Yield_pH_N_interaction",
            "Yield_per_Height", "Harvest_Index_Calc", "Nitrogen_Use_Efficiency",
            "Water_Use_Efficiency", "Yield_per_Plot_Calc", "Yield_per_Hectare_Calc",
            "Yield_per_Hectare_log", "Yield_category",
        ]
        ...
        r = LayerResult(
            layer=self.LAYER, agent="FeatureAgent",
            rows_in=200, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            new_cols=new_cols,
    ...
    def test_training_agent_leakage_guard(self):
        ...
        r = LayerResult(
            layer=self.LAYER, agent="TrainingAgent",
            rows_in=200, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
...
class TestLayer6Decision:
    ...
    def test_fuzzy_logic_agent(self):
        ...
        assert contract.status == "success"
        r = LayerResult(
            layer=self.LAYER, agent="FuzzyLogicAgent",
            rows_in=50, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            passed=True,
        )
    ...
    def test_recommendation_agent(self):
        ...
        assert contract.status == "success"
        r = LayerResult(
            layer=self.LAYER, agent="RecommendationAgent",
            rows_in=50, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            passed=True,
        )
    ...
    def test_benchmark_agent(self):
        ...
        assert contract.status == "success"
        r = LayerResult(
            layer=self.LAYER, agent="BenchmarkAgent",
            rows_in=100, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            passed=True,
        )
...
class TestLayer7Meta:
    ...
    def test_ready_reckoner_agent(self):
        ...
        assert contract.status == "success"
        r = LayerResult(
            layer=self.LAYER, agent="ReadyReckonerAgent",
            rows_in=100, rows_out=len(result_df),
            cols_in=len(df.columns), cols_out=len(result_df.columns),
            passed=True,
        )
...
def _print_efficiency_summary(results: list[LayerResult]):
    ...
    for r in results:
        ...
        print(f"{r.layer:<20} {r.agent:<30} {r.throughput_rps:>8.1f} {col_delta:>8} "
